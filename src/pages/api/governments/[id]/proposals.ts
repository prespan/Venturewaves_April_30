// pages/api/governments/[id]/proposals.ts
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    console.log(`✨ Formatted ${formattedProposals.length} proposals for government ${id}`);

    res.status(200).json({ proposals: formattedProposals });
  } catch (error) {
    console.error('❌ Government Proposals API Error:', error);
    res.status(500).json({ 
