// pages/api/corporate/[id]/proposals.ts
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    console.log(`✨ Formatted ${formattedProposals.length} proposals for corporate ${id}`);

    res.status(200).json({ proposals: formattedProposals });
  } catch (error) {
    console.error('❌ Corporate Proposals API Error:', error);
    res.status(500).json({ 
